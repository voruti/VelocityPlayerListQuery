...
public class VelocityPlayerListQuery {
  ...
  @Inject ServerPingProcessorRegistry serverPingProcessorRegistry;
  ...
  public void onProxyInitialization(ProxyInitializeEvent ignored) {
    ...
    this.hooks
        .sayanVanish()
        .ifPresent(unused -> this.logger.info("SayanVanish found, enabling vanish support"));
  ...
  public EventTask onServerPing(final ProxyPingEvent event) {
    ...
    return EventTask.async(
        () -> {
          ...
          // Version and description (aka. MOTD) are both required to create a new ServerPing
          // instance.
          ...
          try {
            this.processPing(serverPing).ifPresent(event::setPing);
  ...
  private Optional<ServerPing> processPing(@NonNull final ServerPing serverPing) {
    ...
    for (ServerPingProcessor processor :
        this.serverPingProcessorRegistry.serverPingProcessorList()) {
    ...
    if (builder.getVersion() == null) {
      throw new InvalidServerPingException(
          "Missing version info in server ping even after processing");
    }

    return isModified ? Optional.of(builder.build()) : Optional.empty();
  }
}
